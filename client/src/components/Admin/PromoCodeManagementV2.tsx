...
// Interface mise à jour pour correspondre aux données réelles de l'API
interface PromoCode {
  id: number;
  code: string;
  discountType: "percentage" | "fixed";
  discountValue: number; // au lieu de discount
  maxUses: number; // au lieu de usageLimit
  maxUsesPerUser: number;
  usedCount: number; // au lieu de usageCount
  isActive: boolean; // au lieu de active
  startDate: string | null;
  endDate: string | null;
  minimumOrderAmount: number; // au lieu de minOrder
  createdAt: string;
  // maxDiscount n'existe pas dans l'API
}

// Schema adapté à la structure de l'API
const promoCodeSchema = z.object({
  code: z.string().min(3, "Le code doit contenir au moins 3 caractères"),
  discountValue: z.coerce.number().min(1, "La réduction doit être d'au moins 1"),
  discountType: z.enum(["percentage", "fixed"]),
  minimumOrderAmount: z.coerce.number().min(0, "La commande minimum doit être d'au moins 0"),
  maxDiscount: z.coerce.number().min(0, "La réduction maximum doit être d'au moins 0").optional(),
  maxUses: z.coerce.number().min(0, "La limite d'utilisation doit être d'au moins 0"),
...
const PromoCodeManagement = () => {
  ...
  const addPromoCodeMutation = useMutation({
    mutationFn: async (promoData: z.infer<typeof promoCodeSchema>) => {
      ...
      // Si l'utilisation est illimitée, définir maxUses à 0
      if (promoData.unlimited) {
        promoData.maxUses = 0;
      }

      // Supprimer la propriété unlimited avant l'envoi
      const { unlimited, maxDiscount, ...dataToSend } = promoData;
  ...
  const handleEditPromo = (promo: PromoCode) => {
    ...
    // Set unlimited state based on maxUses value
    const isUnlimited = promo.maxUses === 0;
    ...
    form.reset({
      ...
      maxDiscount: 0, // on n'a pas cette valeur depuis l'API
      maxUses: isUnlimited ? 0 : promo.maxUses,
              ...
              {filteredPromoCodes.map((promo) => (
                ...
                >
                  <div className="flex items-center justify-between p-3 bg-[#132743] border-b border-[#1d3a56]">
                    ...
                    <Badge
                      className={`${getStatusBadgeClass(promo.isActive)} ml-2`}
                    >
                      {promo.isActive
                  ...
                  <div className="p-3 space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-primary">
                        {getDiscountDisplay(promo.discountValue, promo.discountType)}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-white">{promo.minimumOrderAmount} MAD</span>
                    </div>
                ...
                <TableBody>
                  {filteredPromoCodes.map((promo) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge className={getStatusBadgeClass(promo.isActive)}>
                          {promo.isActive
                            ? translate("admin.active")
                            : translate("admin.inactive")}
                        </Badge>
