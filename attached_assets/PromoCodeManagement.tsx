...
const promoCodeSchema = z.object({
  ...
  minOrder: z.coerce.number().min(0, "Minimum order must be at least 0"),
  maxDiscount: z.coerce.number().min(0, "Maximum discount must be at least 0"),
  usageLimit: z.coerce.number().min(0, "Usage limit must be at least 0"),
...
const PromoCodeManagement = () => {
  ...
  const { data: promoCodes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const response = await apiRequest("GET", "/api/promo-codes", undefined, customHeaders);
  ...
  const addPromoCodeMutation = useMutation({
    mutationFn: async (promoData: z.infer<typeof promoCodeSchema>) => {
      ...
      const response = await apiRequest("POST", "/api/promo-codes", dataToSend, headers);
  ...
  const updatePromoCodeMutation = useMutation({
    mutationFn: async ({ id, promoData }: { id: number, promoData: z.infer<typeof promoCodeSchema> }) => {
      const headers: Record<string, string> = {
        'X-User-Id': localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user') || '{}').id.toString() : '',
        'X-User-Role': 'admin'
      };
      ...
      const response = await apiRequest("PUT", `/api/promo-codes/${id}`, dataToSend, headers);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/promo-codes"] });
          ...
          <>
            ...
            <div className="md:hidden space-y-4">
              {filteredPromoCodes.map((promo) => (
                <div key={promo.id} className="bg-[#0a0f1a] rounded-lg overflow-hidden border border-[#1d3a56]">
                  <div className="flex items-center justify-between p-3 bg-[#132743] border-b border-[#1d3a56]">
                    ...
                    <Badge className={`${getStatusBadgeClass(promo.active)} ml-2`}>
                      {promo.active 
                  ...
                  <div className="p-3 space-y-2">
                    <div className="flex justify-between items-center">
                      <div className="text-sm text-gray-400">{translate("admin.discount")}:</div>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="text-sm text-gray-400">{translate("admin.minOrder")}:</div>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="text-sm text-gray-400">{translate("admin.usageLimit")}:</div>
                      <span className="text-white">
                        {promo.usageLimit === 0 
                    ...
                    <div className="flex justify-between items-center">
                      <div className="text-sm text-gray-400">{translate("admin.validity")}:</div>
                      <span className={`text-sm ${isExpired(promo.endDate) ? 'text-[#E63946]' : 'text-green-400'}`}>
            ...
            <div className="hidden md:block rounded-[0.75rem] overflow-x-auto">
              <Table>
                <TableHeader className="bg-[#0a0f1a]">
                  <TableRow>
                    <TableHead className="text-primary">{translate("admin.code")}</TableHead>
                    <TableHead className="text-primary">{translate("admin.discount")}</TableHead>
                    <TableHead className="text-primary">{translate("admin.minOrder")}</TableHead>
                    <TableHead className="text-primary">{translate("admin.maxDiscount")}</TableHead>
                    <TableHead className="text-primary">{translate("admin.usage")}</TableHead>
                    <TableHead className="text-primary">{translate("admin.validity")}</TableHead>
                    <TableHead className="text-primary">{translate("admin.status")}</TableHead>
                    <TableHead className="text-primary text-right">{translate("admin.actions")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredPromoCodes.map((promo) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {promo.discountType === 'percentage' ? `${promo.maxDiscount} MAD` : '-'}
                      </TableCell>
                      <TableCell>
                        {promo.usageLimit === 0 ? (
                          <span className="text-gray-400">{translate("admin.unlimited")}</span>
                        ) : (
                          <span>{promo.usageCount} / {promo.usageLimit}</span>
                        )}
                      </TableCell>
                      <TableCell>
                        <span className={isExpired(promo.endDate) ? 'text-[#E63946]' : ''}>
                          {formatDate(promo.startDate)} - {formatDate(promo.endDate)}
                        </span>
                      </TableCell>
                      <TableCell>
        ...
        <DialogContent className="bg-[#132743] text-white border-[#B8860B] max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-cairo text-2xl">
              {selectedPromo ? translate("admin.editPromoCode") : translate("admin.addPromoCode")}
            </DialogTitle>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      >
                        <FormControl>
                          <SelectTrigger className="bg-[#0a0f1a] border-[#B8860B] text-white">
                            <SelectValue placeholder={translate("admin.selectDiscountType")} />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent className="bg-[#0a0f1a] border-[#B8860B] text-white">
                          <SelectItem value="percentage">{translate("admin.percentage")}</SelectItem>
                          <SelectItem value="fixed">{translate("admin.fixed")}</SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                    </FormItem>
              ...
              <FormField
                ...
                name="unlimited"
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border border-[#B8860B]/50 p-3 bg-[#0a0f1a]">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base">{translate("admin.unlimitedUses")}</FormLabel>
                      <FormDescription className="text-xs text-gray-400">
                        {translate("admin.unlimitedUsesDescription")}
                      </FormDescription>
                    </div>
                    <FormControl>
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          // If unlimited, set usageLimit to 0, otherwise to 1
                          form.setValue("usageLimit", checked ? 0 : 1);
                        }}
                      />
                    </FormControl>
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border border-[#B8860B]/50 p-3 bg-[#0a0f1a]">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base">{translate("admin.active")}</FormLabel>
                      <FormDescription className="text-xs text-gray-400">
              ...
              <DialogFooter className="flex justify-between sm:justify-between mt-6">
                ...
                <Button 
                  type="submit" 
                  className="bg-primary hover:bg-primary/90 text-background"
                  disabled={addPromoCodeMutation.isPending || updatePromoCodeMutation.isPending}
                >
                  {(addPromoCodeMutation.isPending || updatePromoCodeMutation.isPending) ? (
                    <span className="flex items-center">
                    ...
                    <>
                      ...
                      {selectedPromo ? translate("admin.saveChanges") : translate("admin.createPromoCode")}
                    </>
                  )}
                </Button>
              </DialogFooter>
